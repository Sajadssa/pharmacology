...
//  console.log(event.target);
  ...
  console.log(id);
 const _removedItem=cart.find((c)=>c.id===parseInt(removeItem.dataset.id));
 ...
 cartContent.removeChild(removeItem.parentElement);
  ...
  //1.get item from cart
  const substractedItem = cart.find((cItem) => cItem.id == substractQuantity.dataset.id);
  
  if(substractedItem.quantity===1){
    
    this.removeItem(substractedItem.id);
    cartContent.removeChild(substractQuantity.parentElement.parentElement);
  ...
  substractQuantity.previousElementSibling.innerText = substractedItem.quantity;
...
document.addEventListener("DOMContentLoaded", () => {
 ...
 ui.setupApp();
  ...
  (!cart.length) ? cartStatus.style.opacity = "1":cartItems.style.opacity = "1";
